...
vec2 mouse = vec2(sin(time) / 48.0, cos(time) / 48.0);
...
void main() {
    vec2 uv2 = 2.0 * gl_FragCoord.xy / vec2(512.0) - 1.0;
    vec2 uvs = uv2 * vec2(512.0) / 512.0;
    float time2 = time;
    float speed = speed2;
    speed = 0.01 * cos(time2 * 0.02 + 3.1416 / 4.0);
    ...
    // Применяем настраиваемую прозрачность
    vec4 finalColor = forCol2 + vec4(backCol2, 1.0);
