 ...
 */
package fear.dlc.ui.clickGui.components.settings.modeListSetting.window;
...
import fear.dlc.ui.clickGui.components.settings.modeListSetting.ModeListSettingHelper;
...
public class ModeListSettingWindowComponent
extends WindowLayer {
    ...
    public ModeListSettingWindowComponent(ModeListSetting modeListSetting) {
        ...
        this.components.addAll(ModeListSettingHelper.values(modeListSetting));
    }

    public void init() {
        this.size(((Float)this.modeListSetting.asStringList().stream().map(ModeListSettingWindowComponent::lambda$init$0).reduce(0f, Float::max)).floatValue(), (float)this.modeListSetting.asStringList().size() * 15f);
    }

    public ModeListSettingWindowComponent render(class_332 context, int mouseX, int mouseY, float delta) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (Math.isHover(mouseX, mouseY, this.getX(), this.getY(), this.getWidth(), this.getHeight())) {
            this.components.forEach(ModeListSettingWindowComponent::lambda$mouseClicked$2 /* captured: mouseX, mouseY, button */);
            return true;
        }
